package com.sangkwon.sangkwonplatform.map.service;

import com.sangkwon.sangkwonplatform.map.dto.request.AptSearchRequest;
import com.sangkwon.sangkwonplatform.map.dto.response.AptResponse;
import com.sangkwon.sangkwonplatform.map.entity.Apt;
import com.sangkwon.sangkwonplatform.map.repository.AptRepository;
import org.junit.jupiter.api.Test;
import org.junit.jupiter.api.extension.ExtendWith;
import org.mockito.InjectMocks;
import org.mockito.Mock;
import org.mockito.junit.jupiter.MockitoExtension;

import java.util.List;

import static org.assertj.core.api.Assertions.assertThat;
import static org.mockito.Mockito.mock;
import static org.mockito.Mockito.verify;
import static org.mockito.Mockito.when;

@ExtendWith(MockitoExtension.class)
class AptServiceTest {

    @Mock
    AptRepository aptRepository;

    @InjectMocks
    AptService aptService;

    @Test
    void 분기_상권으로_조회해서_DTO로_변환한다() {
        Apt a = mock(Apt.class);
        when(a.getTrdarCd()).thenReturn("3110001");
        when(a.getAptHshldCo()).thenReturn(1200L);
        when(aptRepository.search("20242", "3110001")).thenReturn(List.of(a));

        List<AptResponse> result = aptService.getApts(
                new AptSearchRequest("20242", "3110001"));

        assertThat(result).hasSize(1);
        assertThat(result.get(0).trdarCd()).isEqualTo("3110001");
        assertThat(result.get(0).aptHshldCo()).isEqualTo(1200L);
        verify(aptRepository).search("20242", "3110001");
    }

    @Test
    void 필터가_없으면_null로_전체_조회한다() {
        when(aptRepository.search(null, null)).thenReturn(List.of());

        List<AptResponse> result = aptService.getApts(
                new AptSearchRequest(null, null));

        assertThat(result).isEmpty();
        verify(aptRepository).search(null, null);
    }
}
